...
for file in $tmp_dir/staging/*.fastq.gz ; do mv "$file" "$(echo "$file" | sed -r 's/[A-Z]{9}_[A/G/T/C]{8}-[A/G/T/C]{8}_//')" ; done
...
for sample in $(find $tmp_dir/staging/ -type f -name "*.fastq.gz" | while read F; do basename $F | rev | cut -c 18- | rev; done | sort | uniq); do
    ...
    cat "$tmp_dir/staging/$sample"_L00*_R1.fastq.gz > $project_dir/fastq/jejunum_rna/"$sample".fastq.gz
...
rm -rf $project_dir/fastq_raw/jejunum_rna
